...
export const RedemptionManager: React.FC = () => {
  ...
  const [canRedeem, description] = total.collateralRatioIsBelowMinimum(price)
    ? [
      false,
      <ErrorDescription>
        You can't redeem USDM when the total collateral ratio is less than{" "}
    ...
    : lusdAmount.gt(lusdBalance)
      ...
      : [
        true,
        <ActionDescription>
          You will receive <Amount>{ethAmount.sub(ethFee).prettify(4)} {collateral}</Amount> in exchange for{" "}
          <Amount>
