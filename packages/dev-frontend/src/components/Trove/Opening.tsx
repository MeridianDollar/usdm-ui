...
const GAS_ROOM_ETH = Decimal.from(0.005);

export const Opening: React.FC = () => {
  const { collateral: collateral_tag } = useLiquity();
  ...
  return (
    <Card>
      ...
      <Box sx={{ p: [2, 3] }}>
        ...
        <StaticRow
          ...
          infoIcon={
            <InfoIcon
              tooltip={
                <Card variant="tooltip" sx={{ width: "240px" }}>
                  The total amount of USDM your Trove will hold.{" "}
                  {isDirty && (
                    <>
                      You will need to repay {totalDebt.sub(LUSD_LIQUIDATION_RESERVE).prettify(2)}{" "}
                      USDM to reclaim your collateral ({LUSD_LIQUIDATION_RESERVE.toString()} USDM
        ...
        {description ?? (
          <ActionDescription>
            Start by entering the amount of {collateral_tag} you'd like to deposit as collateral.
          </ActionDescription>
        )}
